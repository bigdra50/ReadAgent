...
- **ヘッダーのボタンに文字を置かない。** 日本語のラベルは1文字ずつ折り返すため、
  幅が詰まった瞬間にヘッダーが崩れる。アイコン＋ホバーの吹き出しにしてある。
- **狭い画面のブレークポイントは CSS と TS の2箇所にある。** `styles.css` のメディアクエリと
  `lib/media.ts` の `NARROW_QUERY` は必ず同じ値に保つ。ずれると
  「CSS 上はオーバーレイなのに、状態は並列表示のまま」になる。
...
- **ノートの設定**（`updateMode` / `granularity` / `notePath` / `maxContextChars`）は
  サーバー側で解決する。設定ファイル → 環境変数 → 問い合わせ単位の上書き の後勝ち。
- **表示設定**（ペインの幅など）はブラウザの `localStorage`。ADR-0010。
  端末ごとの見え方であって、書籍やノートの内容には関わらないため混ぜない。
