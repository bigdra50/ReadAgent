...
![本文を選択して質問すると回答がストリーミングで返り、その内容が読書ノートへ追記される](docs/images/demo.gif)

> このページの画面写真と動画は、**エージェントの応答を差し替えた状態**で撮っています。
> 実際の Claude 認証を通した確認ができていないためで、文面はダミーです
...
![左に目次、中央にPDF本文、右に選択範囲と質問欄が並んだ3ペインの画面](docs/images/reading.png)

右ペインの「選択範囲」には、サーバー側で切り出した引用と `p.1 [4, 60)` のような
ページ内オフセットが出ます。ブラウザの選択文字列は使いません
（[docs/spike-pdf-text-anchor.md](docs/spike-pdf-text-anchor.md)）。

サーバーは `127.0.0.1` にのみバインドします。認証が入るまで外に出さないでください（[ADR-0004](docs/adr/0004-runtime-shell.md)）。

### 画面

ヘッダーのボタンはアイコンで、ホバーとキーボードのフォーカスで名前とショートカットが出ます。
...
ペインの境界はドラッグでも矢印キーでも動かせます。幅・拡大率・配色・開いていたペイン・
書籍ごとの続きのページはブラウザに保存されます（[ADR-0010](docs/adr/0010-browser-preferences.md)）。

画面が狭いときは目次とノートがオーバーレイになり、本文が隠れないよう排他で表示されます。

![狭い画面で読書ノートが本文の上にオーバーレイ表示されている](docs/images/narrow.png)

ノートのエントリをクリックすると、記録した箇所の本文へ戻ります。
横断検索の結果からは、別の書籍の該当箇所へも飛べます。

![「ガード」で検索した結果に、2冊の書籍のノートが並んでいる](docs/images/search.png)

ノートペインの「読み直してまとめる」で、溜まったノートの要約を書き足せます。
既存の記録は書き換えず、末尾に追記します（[ADR-0009](docs/adr/0009-note-reorganization.md)）。
ノートに含まれる Mermaid の図は、その場で描画されます。

![読書ノートのペインに、まとめとMermaidの図、質問と引用のエントリが並んでいる](docs/images/notes.png)

目次にページ情報がある PDF では、**章を選んでその範囲だけ**まとめられます。

![ノートペインの上部で「Chapter 2 Guards (p.5–8)」を選び、その範囲だけをまとめようとしている](docs/images/chapters.png)
...
チャットペインの「詳細設定」からは、**その問い合わせだけに効く一時上書き**ができます。
設定ファイルには書き込みません。

![質問欄の下に開いた詳細設定で、更新・粒度・文脈の上限・ノートの位置・モデルを指定している](docs/images/settings.png)
